...
def apply_theme(gender_choice):
    ...
    st.markdown(f"""
        ...
        h1, h2, h3, p {{ color: white !important; text-align: right; }}
        </style>
    """, unsafe_allow_html=True)

# القائمة الجانبية للبيانات الأساسية (فكرة BetterMe)
with st.sidebar:
...
# نظام الأسئلة التحليلي (فكرة Intermittent Fasting)
with st.container():
    ...
    if st.button("تحليل البيانات وإنشاء الخطة 🔥"):
        with st.status("جاري دمج البيانات وتحليل الأهداف...", expanded=True) as status:
            ...
            time.sleep(1)
            st.write(f"✅ تم تحديد تمارين مخصصة لـ {focus_area}...")
            status.update(label="تم تجهيز خطتك الشخصية!", state="complete")
        
        st.balloons()
        
        # عرض النتائج بطريقة BetterMe
        st.markdown("---")
        ...
        st.info(f"💡 نصيحة نونا: بما أن هدفك هو {target_area}، ركزي على تمارين الكارديو الصباحية قبل أول وجبة!")

# قسم الاشتراكات (الأسعار اللي حددتيها)
st.markdown("---")
st.subheader("💎 ترقية الحساب (VIP)")
plans = {"تجريبي (1.5 د.ك)": 1.5, "شهري (2.5 د.ك)": 2.5, "3 شهور (7.5 د.ك)": 7.5, "سنة (30 د.ك)": 30}
...
if st.button("اشترك الآن بأمان 💳"):
    st.write(f"جاري تحويلك لبوابة الدفع لدفع {plans[selected_plan]} دينار كويتي")
